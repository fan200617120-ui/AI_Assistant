...
# ---------- 提取思考过程（简化）----------
def extract_content_simple(text):
    text = text.strip()
    if "<think>" in text:
    ...
    for i, line in enumerate(lines):
        ...
        if (line_stripped.startswith(('def ', '```', '### ', '# ', '- ', '1. ', '* ')) or
                '以下是' in line_stripped[:20] or '下面是' in line_stripped[:20]):
            if i > 0:
                split_point = i
                break
    if split_point > 0:
...
def stream_response(message, image, model, temp, tokens, history):
    ...
    user_content = f"[{timestamp}] 用户：{message}"
    if image is not None:
        user_content += " [附图片]"
...
开始对话："""
                ...
                with gr.Column(scale=1, min_width=250):
                    ...
                    image_input = gr.Image(
                        label="仅多模态模型（qwen3.5:4b/9b, qwen3-vl:8b）支持识别",
                        type="filepath",
                        height=180
                    )
                    gr.Markdown("---")
