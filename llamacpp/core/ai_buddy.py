...
class ConfigValidator:
    ...
    def validate_personality_config(config: dict) -> Tuple[bool, Optional[str]]:
        required_fields = ["name", "personality", "mood", "style"]
        for field in required_fields:
            if field not in config:
                return False, f"性格配置缺少必需字段: {field}"
            if not config[field] or not isinstance(config[field], str):
                return False, f"字段 {field} 必须是非空字符串"
        if len(config["name"]) > 50:
            return False, "名称不能超过50个字符"
        if len(config["personality"]) > 200:
            return False, "性格描述不能超过200个字符"
        if len(config["style"]) > 500:
            return False, "风格描述不能超过500个字符"
        return True, None
    ...
    def _load_config(self) -> Dict:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                is_valid, error_msg = ConfigValidator.validate_personality_config(loaded_config)
                if not is_valid:
                    print(f"性格配置验证失败: {error_msg}, 使用默认配置")
...
class StreamResponseParser:
    ...
    def parse_chunk(self, chunk_data: dict) -> Dict[str, str]:
        ...
        if reasoning:
            self.current_thought += reasoning
...
class AIBuddy:
    ...
    def build_messages_with_memory(self, message: str, image_data_url: Optional[str] = None) -> List[Dict]:
        personality_config = self.personality.config
        current_date = datetime.now().strftime("%Y年%m月%d日")
        
        system_content = f"""今天是 {current_date}。你叫{personality_config['name']}，性格{personality_config['personality']}，心情{personality_config['mood']}。
...
你的任务是与用户进行自然、温暖的对话，提供情绪价值和支持。请用中文回答。"""
        ...
        if image_data_url and self.is_multimodal(self.config.default_model):
            user_content = [
    ...
    def stream_chat(self, message: str, image_path: Optional[str], model: str = None,
                    ...
                    n_gpu_layers: int = -1, session_id: str = None) -> Generator:
        if not session_id:
        ...
        if image_path and os.path.exists(image_path):
            if self.is_multimodal(model or self.config.default_model):
                ...
                yield "", f"当前模型 {model} 不支持图片识别，请选择支持多模态的模型（如 qwen、llava 等）", False
                self._unregister_session(session_id)
                return

        messages = self.build_messages_with_memory(message, image_data_url)
        ...
        try:
            ...
            yield "", "无法连接到 llama.cpp 服务，请确保已运行 llama-server（例如：llama-server.exe -m model.gguf --host 0.0.0.0 --port 8080）", False
...
.thoughts-details summary:hover {
    background-color: var(--color-background-secondary);
}

.thoughts-details[open] summary {
...
def create_chat_interface(ai_buddy, personality_config, config):
    with gr.Tab("聊天", id="chat-tab"):
            ...
            with gr.Column(scale=3):
                chat_history = gr.HTML(
                    value=render_markdown(
                        ...
                        f"### 🎯 小贴士：\n"
                        f"- 在右侧面板可以 **切换模型**、**调整性格**、设置 **温度** 和 **回答长度**。\n"
        ...
        def start_streaming(message, image, history_html, model, temperature, max_tokens, gpu_layers, current_session_id):
            ...
            for full_response, status_msg, is_streaming in ai_buddy.stream_chat(
                    clean_message, image, model, temperature, max_tokens, gpu_layers, new_session_id):
                if full_response:
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    ai_html = f'''
        ...
        def update_personality(personality_str, current_html):
            personality_map = {
        ...
        send_btn.click(
            ...
            inputs=[user_input, chat_image_input, chat_history, model_choice,
                    temperature_slider, max_tokens_slider, gpu_layers_slider, session_state],
