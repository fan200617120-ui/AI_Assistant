...
# -*- coding: utf-8 -*-
"""
bilingual_subtitle_translator_llama.py - 双语字幕翻译工具 (llama.cpp 后端) v2.0
特性：
- 异步翻译 + 并发控制，界面不卡顿
- GPU 层数滑块，动态调整推理负载
- 翻译缓存持久化（JSON）
            ...
            try:
                with open(self.cache_file, 'r', encoding='utf-8') as f:
                    ...
                    for k_str, v in data.items():
                        # 缓存键原本是 (text, lang, model, temp, gpu_layers)，这里简化存储为 text_lang_model_temp
                        # 实际可根据需求设计，此处简单恢复为元组（需反序列化）
                        parts = k_str.split('|||')
                        if len(parts) == 5:
...
翻译："""
    ...
    async def translate_subtitles_async(self, subtitles: List[Dict], target_lang: str,
                                        ...
                                        temperature: float, gpu_layers: int) -> List[Dict]:
        """异步批量翻译字幕"""
            ...
            for subtitle in subtitles:
                ...
                if original_text.strip():
                    tasks.append(self._async_translate_one(
                        session, original_text, target_lang, model, temperature, gpu_layers
                    ))
                else:
...
class TranslatorUI:
    ...
    def create_interface(self):
        with gr.Blocks(title="双语字幕翻译工具 (llama.cpp 异步版)", theme=gr.themes.Default()) as demo:
            gr.Markdown("""# 双语字幕翻译工具 (llama.cpp 异步版)
**使用本地 llama.cpp 服务翻译字幕，支持 GPU 层数调整、缓存持久化、异步并发**
""")
            ...
            with gr.Tabs():
                ...
                with gr.Tab("单文件翻译"):
                    with gr.Row():
                        with gr.Column(scale=1):
                            ...
                            with gr.Group():
                                ...
                                max_concurrent = gr.Slider(1, 10, value=3, step=1, label="最大并发数", info="同时翻译的句子数，值越大越快但可能加重服务器负载")
                    ...
                    refresh_models_btn.click(self.refresh_models, outputs=[model_select])

                    # 异步翻译调用
                    translate_btn.click(
                        fn=self.translate_subtitles_async_wrapper,
                        inputs=[input_text, model_select, target_lang, bilingual_style,
                                temperature, gpu_layers, max_concurrent],
                    ...
                    preview_btn.click(
                        ...
                        inputs=[input_text, model_select, target_lang, bilingual_style, temperature, gpu_layers],
                ...
                with gr.Tab("批量翻译"):
                    ...
                    batch_translate_btn.click(
                        ...
                        inputs=[batch_files, batch_model, batch_target_lang, batch_format,
                                batch_temperature, batch_gpu_layers, batch_style],
...
""")
    ...
    async def translate_subtitles_async_wrapper(self, content, model, target_lang, bilingual_style,
                                                temperature, gpu_layers, max_concurrent, progress=gr.Progress()):
        ...
        try:
            ...
            translated_subtitles = await self.translator.translate_subtitles_async(
                subtitles, target_lang, model, bilingual_style, temperature, int(gpu_layers)
            )
    ...
    async def preview_translation_async_wrapper(self, content, model, target_lang,
                                                bilingual_style, temperature, gpu_layers):
        if not content.strip():
            return "请输入或上传字幕内容"
        if not model:
        ...
        try:
            ...
            translated_preview = await self.translator.translate_subtitles_async(
                preview_subtitles, target_lang, model, bilingual_style, temperature, int(gpu_layers)
            )
    ...
    async def batch_translate_async_wrapper(self, files, model, target_lang, output_format,
                                            temperature, gpu_layers, bilingual_style):
        if not files:
            return "请选择要翻译的文件"
        if not model:
            return "请先选择翻译模型"
...
def main():
    print("=" * 70)
    print("双语字幕翻译工具 v2.0 (llama.cpp 异步版 + GPU层数控制)")
    print("=" * 70)
...
if __name__ == "__main__":
    import requests  # 注意：异步翻译用 aiohttp，同步检查状态仍用 requests
    main()